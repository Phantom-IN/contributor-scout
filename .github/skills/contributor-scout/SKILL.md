...
argument-hint: '<profile|full|security|performance|features|validate <id>|refresh> [args]'
---
...
Invoke as `/contributor-scout <mode> [args]` or in natural language ("use the
contributor-scout skill in profile mode"). Both are equivalent - the mode is
...
For non-trivial repositories, delegate each reviewer to its custom agent
...
**Custom agents** (`.github/agents/` directory): `repository-intelligence`,
